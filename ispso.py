...
class ispso:
    ...
    def evaluate_f(self, s):
        ...
        for i in range(s.S):
            ...
            if f[i] < self.pbest[i, self.s.D] or (
                f[i] == float("inf") and self.pbest[i, self.s.D + 1] == float("inf")
            ):
                self.pbest[i] = [self.x[i], f[i]]
                if f[i] < self.gbest[self.s.D] or (
                    f[i] == float("inf") and self.gbest[self.s.get("D")] == float("inf")
                ):
    ...
    def update_v(self):
        ...
        self.f = f
        tmp = pd.DataFrame(
            {"x": self.x.flatten(), "f": f, "v": self.V, "age": self.age}
        )
